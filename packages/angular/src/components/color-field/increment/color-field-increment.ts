...
/**
 * Steps the field up, applied to a native `<button>`.
 *
...
@Directive({
  ...
  exportAs: "urcColorFieldIncrement",
  host: {
    "[attr.type]": "'button'",
    "[attr.tabindex]": "'-1'",
...
export class ColorFieldIncrement {
  private readonly root = inject(ColorFieldRoot);
  private readonly destroyRef = inject(DestroyRef);

  /** A consumer's own `aria-label` wins; "Increase" is only a fallback. */
  protected readonly label
    = inject(new HostAttributeToken("aria-label"), { optional: true }) ?? "Increase";
  ...
  protected readonly isDisabled = computed(
    () => this.root.isDisabled() || this.root.isReadOnly() || this.root.isIncreaseDisabled(),
  );
