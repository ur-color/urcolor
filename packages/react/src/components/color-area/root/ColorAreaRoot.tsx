...
export const ColorAreaRoot = forwardRef<HTMLDivElement, ColorAreaRootProps>(
  function ColorAreaRoot(props, ref) {
    ...
    const handlePointerDown = useCallback((event: React.PointerEvent) => {
      ...
      if (thumbRef.current && thumbRef.current.contains(target)) {
    ...
    const STEP_KEY_DELTAS: Record<string, { axis: "x" | "y"; sign: number }> = {
    ...
    function handleBoundaryKey(axis: "x" | "y", boundaryValue: number) {
      const value = internalValue[valueIndexToChange];
      if (!value) return;
    ...
    const ctxValue = useMemo<ColorAreaContextValue>(() => ({
      ...
      colorRef,
      isDragging,
    }), [disabled, minX, maxX, minY, maxY, internalValue, valueIndexToChange, isSlidingFromLeft, isSlidingFromTop, thumbAlignment, colorSpace, xChannelKey, yChannelKey, colorRef, isDragging]);
