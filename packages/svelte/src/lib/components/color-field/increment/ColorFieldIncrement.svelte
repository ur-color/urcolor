...
<script lang="ts">
  ...
  let { class: className, style, children, child, ...rest }: ColorFieldIncrementProps = $props();

  const context = colorFieldContext.get();

  const isDisabled = $derived(context.disabled || context.readOnly || context.isIncreaseDisabled);
