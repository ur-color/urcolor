 ...
 * unsupported.
 */
export const CSS_GRADIENT_ROOT_STYLE = "position:absolute;inset:0;width:100%;height:100%;pointer-events:none;";
/** The inline style that paints one layer. */
export function cssLayerStyle(layer) {
 ...
 */
export function warnNoCssRecipe(name) {
    if (warned[name])
        return;
    if (typeof process !== "undefined" && process.env && process.env.NODE_ENV === "production")
        return;
    warned[name] = true;
    console.warn(`[urcolor] ${name} has renderer="css", but this color space and channel `
 ...
 */
export function resolveCssGradient(renderer, name, hasChild, build) {
    if (renderer === "canvas" || hasChild)
        return null;
    const layers = build();
    if (!layers || layers.length === 0) {
        if (renderer === "css")
            warnNoCssRecipe(name);
        return null;
    }
