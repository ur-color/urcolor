...
**Files:**
- Modify: `packages/core/bench/report.ts` (the generator's prose and version table)
...
- [ ] **Step 1: Fix the generator's attribution — before regenerating**

The Task 8 review found that `docs/guide/benchmarks.md` frames itself entirely around `@urcolor/core` while its "Canvas gradient rendering" section benchmarks `sampleBilinearGrid`, `sampleChannelGrid`, `samplePolarGrid` and `sampleConicRing` — functions that now live in `packages/shared/src/gradient.ts`. The page is generated, so the fix belongs in the generator, not the markdown.

Edit the `out` array in `packages/core/bench/report.ts`. Line 199 currently reads:

```ts
  "How `@urcolor/core` compares to the color libraries people actually reach for.",
```

Change it to name both packages, since the page now measures both:
...
The version table at line 225 lists only core:

```ts
  `| **@urcolor/core** | **${pkg.version}** |`,
```

`pkg` is core's own `package.json`. Add a row for shared beneath it, reading its version from `packages/shared/package.json` the same way `pkg` is read at the top of the file — do not hardcode the number.

The gradient suite's blurb (around line 92) says "urcolor's WebGL entry points are not benchmarked here" — still true, leave it. The intro's claim that the suites live in `packages/core/bench` is also still true: the runner did not move, only the module it imports. Leave the link.
...
Expected: rewrites `docs/guide/benchmarks.md` and `docs/public/benchmarks.json`. Run it on an otherwise idle machine — background load inflates the averages. If the generator throws, your Step 1 edit is at fault; fix it and re-run.

Then read the top of the regenerated `docs/guide/benchmarks.md` and confirm both package rows appear with real version numbers, and that the opening sentence names both packages.
